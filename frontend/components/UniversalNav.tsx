...
export default function UniversalNav() {
  ...
  // If we are on /dashboard, it's the ?tab= param. Otherwise none of these tabs are active.
  const activeTab = pathname === '/dashboard' ? (searchParams.get('tab') || 'dashboard') : '';

  const handleTabClick = (tab: string) => {
    // If not on dashboard, or changing tabs on dashboard, always push to dashboard with tab param
        ...
        <div className="flex items-center h-20 w-full">
          ...
          <div className="flex-1 flex justify-start">
            <Link href="/" className="flex items-center group cursor-pointer select-none">
              <span className="text-3xl font-bold tracking-tight text-white transition-opacity group-hover:opacity-80">Career</span>
              <span className="text-3xl font-black tracking-tight text-[#0F52BA] transition-opacity group-hover:opacity-80">OS</span>
            </Link>
          </div>
                ...
                >
                  <span
                    className={`relative z-10 text-base font-semibold transition-colors capitalize ${
                      activeTab === tab ? 'text-[#FFF]' : 'text-white/70 group-hover:text-white'
                    }`}
                  >
                    {tab.replace('-', ' ')}
                  </span>
          ...
          {/* Right: Profile — shimmer skeleton while session resolves to prevent flash */}
